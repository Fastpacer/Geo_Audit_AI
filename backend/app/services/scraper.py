...
class UnifiedScraper:
    def __init__(self, groq_client=None):
        ...
        self.multi_strategy = MultiStrategyExtractor()
        self.archive_org = ArchiveOrgExtractor()
    ...
    async def extract(self, url: str) -> Dict:
        ...
        # Tier 1: Reader Mode (Fast, precise extraction)
        print(f"[Tier 1] Reader Mode extraction: {url_str[:60]}...")
        result = self.reader.extract(url_str)
        if result and result.get('confidence', 0) > 0.7 and len(result.get('content', '')) > 500:
            print(f"[Tier 1] Reader mode success: {len(result.get('content', ''))} chars")
            return self._normalize(result, url_str)
        
        # Tier 2: Playwright (Browser-based extraction)
        print(f"[Tier 2] Playwright extraction: {url_str[:60]}...")
        try:
            if _use_sync_playwright:
        ...
        # Tier 3: Multi-Strategy + Archive.org (Alternative extraction methods)
        print(f"[Tier 3] Multi-Strategy extraction: {url_str[:60]}...")
        multi_result = None
        
        # Try multi-strategy extraction first
        try:
            multi_result = self.multi_strategy.extract(url_str)
            if multi_result and len(multi_result.get('content', '')) > 300:
                print(f"[Tier 3] Multi-strategy success: {len(multi_result.get('content', ''))} chars")
                return self._normalize(multi_result, url_str)
        except Exception as e:
            print(f"[Tier 3] Multi-strategy failed: {e}")
        
        # Try Archive.org as fallback
        print(f"[Tier 3] Archive.org fallback: {url_str[:60]}...")
        try:
            archive_result = self.archive_org.extract(url_str)
            if archive_result and len(archive_result.get('content', '')) > 300:
                print(f"[Tier 3] Archive.org success: {len(archive_result.get('content', ''))} chars")
                return self._normalize(archive_result, url_str)
        except Exception as e:
            print(f"[Tier 3] Archive.org failed: {e}")
        
        # Tier 4: LLM Reconstruction with comprehensive metadata
        print(f"[Tier 4] LLM reconstruction with comprehensive metadata: {url_str[:60]}...")
        try:
            comprehensive_meta = self.metadata_enricher.extract(url_str)
            enriched = await self.llm_enricher.enrich(comprehensive_meta, "")
            print(f"[Tier 4] LLM enrichment complete")
            return self._normalize(enriched, url_str)
        except Exception as e:
            print(f"[Tier 4] LLM enrichment failed: {e}")
            
            # Final fallback: basic metadata only
            basic_meta = self._extract_metadata_fallback(url_str)
            return self._normalize({
                'title': basic_meta.get('title', ''),
                'description': basic_meta.get('description', ''),
                'content': basic_meta.get('description', ''),
                'headings': [],
                'image': basic_meta.get('image', ''),
    ...
    def _get_tier_from_source(self, source: str) -> str:
        tiers = {
            ...
            'multi_strategy': '3_alternative',
            'archive_org': '3_archived',
            'llm_enriched': '4_ai_enhanced',
            'metadata_only': '4_fallback'
        }
